...
namespace DirectShowNETCF.Helper
{
    public static class DirectShowHelper
    {
        ...
        public static bool ConnectPins(IGraphBuilder graph, IBaseFilter output, IBaseFilter input)
        {
            ...
            LoggerDSCF.Logger.Instance.WriteLog(string.Format("out {0}, in {1}", outputs.Count, inputs.Count));
        ...
        public static bool RenderPins(IGraphBuilder graph, IBaseFilter output)
        {
            bool result = false;
            CPinList pins = new CPinList();
            pins.Assign(output);

            LoggerDSCF.Logger.Instance.WriteLog(string.Format("Pins count {0}", pins.Count));
            for (int i = 0; i < pins.Count; ++i)
            {
                int hr = -1;

                int direction = 0;
                pins[i].QueryDirection(out direction);
                ...
                LoggerDSCF.Logger.Instance.WriteLog(string.Format("Render pin #{0}, result={1}", i, hr));
            }
