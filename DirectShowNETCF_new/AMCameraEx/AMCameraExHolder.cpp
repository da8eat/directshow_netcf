...
{
	FILE * ff = fopen("AMCameraExLog.txt", "a");
	fprintf(ff, "%s\n", msg);
	fclose(ff);
};
...
AMCameraExHolder::AMCameraExHolder() : cca_(0)
{
	...
	{
		//trace__("Error: Cannot get camera filter");
		video.clear();
		graph.clear();
		capGraph.clear();
	...
	{
		//trace__("Error: Cannot add camera filter to graph");
		video.clear();
		graph.clear();
		capGraph.clear();
		return;
...
{
	//trace__("AMCameraEx - init");
	if (!video.isValid())
	{
		//trace__("Error: source is not valid");
	...
	if (!getGrabber())
	{
		//trace__("Error: Cannot get grabber filter");
		return AMRE_GetGrabberError;
	}

	if (FAILED(graph -> AddFilter(AMGrabber.get(), _T("Grabber"))))
	{
		//trace__("Error: Cannot add grabber filter to graph");
	...
	{
		//trace__("Error: Cannot connect capture video source and grabber");
...
bool AMCameraExHolder::getVideoCaptureFilter()
{
	...
	if (!propBag.isValid())
	{
		//trace__("Error: Cannot create prop bag");
		return false;
	}
	...
	{
		//trace__("Error: Cannot load prop bag");
		return false;
	}
